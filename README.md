...
- Models: server-side Vercel AI Gateway tries `openai/gpt-5.4-mini`, then `openai/gpt-5.4`, then a temporary free fallback. The team account must have paid Gateway credits to enable the two OpenAI models and reliable official-source web search. Common stadium questions continue to work from the knowledge snapshot without an AI generation call. The temporary free model may expire; add paid Gateway credits before relying on this site in production.
