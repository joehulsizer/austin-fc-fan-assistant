...
function locationPhrase(section: number | undefined, location: string, language: 'en' | 'es' = 'en') {
  if (!section) return '';
  const match = location.match(/\b(1\d\d|3\d\d)\b/);
  if (!match) return '';
  const target = Number(match[1]);
  if (target === section) return language === 'es' ? ' (en tu sección)' : ' (in your section)';
  if (sectionZone(section) === sectionZone(target)) return language === 'es' ? ' (en la misma zona general del estadio)' : ' (in the same broad stadium area)';
  return '';
}
...
export function groundedFallback(query: string, result: Grounding): string {
  ...
  if (result.route === 'concessions' || result.route === 'drinks') {
    ...
    const list = facts.slice(0, 4).map(f => `• ${f}${locationPhrase(result.context.section, f, es ? 'es' : 'en')}`).join('\n');
...
Verified facts:\n${result.facts.join('\n').slice(0, 6500)}\nPrior conversation:\n${history}`;
  for (const model of ['openai/gpt-5.4-mini', 'openai/gpt-5.4', 'inclusionai/ling-3.0-flash-vl-free']) {
    let emitted = false;
    try {
